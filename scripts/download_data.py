...
NOTE: if you have my retail-analytics-real repo checked out next to this one,
you do NOT need this script — chain/paths.py reads the raw workbook from the
sibling checkout automatically (same file, same bytes; that is part of the
cross-repo consistency story). This script is the fallback for a standalone
checkout. adapted from my retail-analytics-real repo (scripts/download_data.py).

    python scripts/download_data.py

Dataset: Chen, D. (2019). Online Retail II [Dataset]. UCI Machine Learning
Repository. https://doi.org/10.24432/C5CG6D — licensed CC BY 4.0. The raw
xlsx (~45 MB) is deliberately NOT committed and not redistributed here.
"""
...
def main() -> None:
    ...
    sibling = ROOT.parent / "retail-analytics-real" / "data" / "raw" / "online_retail_II.xlsx"
    xlsx = RAW_DIR / "online_retail_II.xlsx"
    if xlsx.exists():
        print(f"[OK] already present: {xlsx}")
        return
    if sibling.exists():
        print(f"[OK] sibling checkout found, no download needed: {sibling}")
        return
    download()
    extract()
